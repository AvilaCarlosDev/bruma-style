...
MODE="${1:-}"
battery_path="${BRUMA_BATTERY_PATH:-}"
...
if [[ -z "$battery_path" || ! -d "$battery_path" ]]; then
  printf '%s\n' 'bruma-style: no battery device detected' >&2
  exit 1
fi

battery_name="${battery_path##*/}"
if [[ ! "$battery_name" =~ ^[A-Za-z0-9_]+$ ]]; then
  printf 'bruma-style: unsupported battery device name: %s\n' "$battery_name" >&2
  exit 1
fi
...
write_thresholds() {
  ...
  printf 'START_CHARGE_THRESH_%s=%s\nSTOP_CHARGE_THRESH_%s=%s\n' \
    ...
    sudo tee /etc/tlp.d/99-bruma-style-battery.conf >/dev/null
}
...
case $MODE in
  ...
  reset)
    ...
    sudo rm -f /etc/tlp.d/99-bruma-style-battery.conf
